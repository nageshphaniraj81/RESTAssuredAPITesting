...
public class RootPathExample2 {
	...
	@Test
	public void test002() {
		given().parameters(parameters).when().get("/yql").then().body("Name", hasItem("USD/INR"));
	}

	// 3 Assert on Multiple Names
	@Test
	public void test003() {
		given().parameters(parameters).when().get("/yql").then().body("Name",
				hasItems("USD/INR", "USD/THB", "USD/BRL"));
	}
	...
	public void test004() {
		given().parameters(parameters).when().get("/yql").then().body("Name", hasItem("USD/INR"))
				.body("id", hasItem("USDCAD")).body("Name", hasItems("USD/INR", "USD/THB", "USD/BRL"));
